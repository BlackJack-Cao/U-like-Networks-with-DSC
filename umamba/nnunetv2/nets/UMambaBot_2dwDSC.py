...
from nnunetv2.nets.DMSK_2d import DMSKModule

class DSCBlock(nn.Module):
    def __init__(self, dim):
        super().__init__()
        self.dmsk_module = DMSKModule(dim)
        self.ttt_layer = TTTLayer(dim)
        
    def forward(self, x):
        x = self.dmsk_module(x)
        x = self.ttt_layer(x)
        return x

class TTTLayer(nn.Module):
    def __init__(self, dim, d_state = 4):
        super().__init__()
        self.dim = dim
...
class UMambaBot(nn.Module):
    def __init__(self,
        ...
        self.dsc_block = DSCBlock(dim = features_per_stage[-1])

        self.decoder = UNetResDecoder(self.encoder, num_classes, n_conv_per_stage_decoder, deep_supervision)

    def forward(self, x):
        skips = self.encoder(x)
        bottleneck = self.mamba_layer(skips[-1])
        skips[-1] = self.dsc_block(bottleneck)
